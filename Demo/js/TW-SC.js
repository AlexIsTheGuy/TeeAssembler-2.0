...
class Tee {
	...
	teeEyesTranslateFunction() {
		...
		this.teeEyes.style.transform = `translate(${this.markerCoord.x - this.teeRender.getBoundingClientRect().x + 0}px, ${this.markerCoord.y - this.teeRender.getBoundingClientRect().y}px)`
	}
	setTeeEyesVariables() {
		this.teeEyesVariables = true

		this.teeRender = document.querySelector('.tee')
		this.line = document.querySelector('.tee .line')
		this.marker = document.querySelector('.tee .marker')
		this.teeEyes = document.querySelector('.eyes')
	}
	lookAtCursor() {
		...
		this.moveTeeEyesFunction = (e) => {
			this.line.style.transform = `translate(-1px, .5px) rotate(${Math.atan2(e.clientY - originCoord.y, e.clientX - originCoord.x) * 180 / Math.PI}deg)`
	...
	lookAt(deg=0) {
		if (this.teeEyesVariables === false) {
			this.setTeeEyesVariables()
		}
